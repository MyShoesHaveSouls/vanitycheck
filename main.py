...
def calculate_final_private_key(salt, seed_private_key):
    """Automated Helper: Calls the native C++ binary to calculate the unblinded private key."""
    binary_path = "./profanity2/profanity2.x64"
    calc_cmd = [binary_path, "--calculate", "--salt", salt, "--secret", seed_private_key]
    
    try:
        result = subprocess.run(calc_cmd, capture_with_stdout=True, stdout=subprocess.PIPE, text=True, check=True)
        for line in result.stdout.splitlines():
            if "Private Key:" in line:
                # Strip out the title and return the raw hex private key cleanly
                return line.split("Private Key:")[-1].strip()
    except Exception as e:
        print(f"[-] Automated calculation failed: {e}")
...
def run_engine():
    ...
    if not os.path.exists(binary_path):
        print(f"[-] Error: '{binary_path}' binary not found. Did you compile it in the subfolder?")
        sys.exit(1)
        
    init_db()
    
    print("=========================================================")
    print("🚀 NVIDIA GTX 1080 HYBRID AUTOMATED PIPELINE")
    print("=========================================================")
    ...
    if choice == "1":
        ...
        criteria_str = "richlist.txt Check"
        ...
        for line in iter(process.stdout.readline, ""):
            ...
            if current_address and current_salt:
                ...
                if is_match:
                    ...
                    # Core Upgrade: Execute the automated unblinding function instantly
